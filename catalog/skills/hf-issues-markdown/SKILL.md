...
1. Validate the requested operation and safe target before reading or writing.
   One issue maps to `<configured-path>/<issue-id>.md`; the file name without
   `.md` must equal frontmatter `id`.
2. Read [references/update-rules.md](references/update-rules.md). Validate the
   ...
   `done`, and `cancelled`.
3. For a new issue, if the target already exists, stop as a duplicate rather
...
4. Required body headings are `## Summary`, `## Acceptance criteria`,
...
5. Before writing, obey the workflow step's approval timing and confirm the file
   has not changed since it was read. A changed file is a conflict: stop and
   reconcile; do not silently overwrite or merge.
6. Apply only the requested local change. Update `updated` from observed workflow
...
Produce the local `issue-file` artifact with the exact changed repository-relative
path, issue ID, operation, and validation evidence. For a duplicate, conflict,
invalid request, or missing approval, report the blocker and exact resume
condition without claiming a change.
